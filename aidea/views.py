...
def get_feedback(request):
    if request.method == 'POST':
        ...
        try:
            ...
            feedback_prompt = f"""
            あなたは、日本の伝統工芸品の魅力を若いクリエイターに伝える、親切で専門知識の豊富なAIデザインパートナーです。
            添付された画像は、「{item_name}」のデザイン案(スケッチ)です。
            
            このデザインについて、以下の構成で具体的なフィードバックをMarkdown形式で返してください。

            ### 1. 素晴らしい点
            まず、このデザインの最も優れている点、魅力的な点を具体的に褒めてください（例：モチーフの選び方、構図の独創性、{item_name}らしさへの挑戦など）。クリエイターの意欲を高めることが重要です。

            ### 2. さらに良くするための提案
            次に、このデザインを製品化する上での**技術的な実現可能性**を評価してください。特に、**複雑な凹凸表現、細かい線、色のグラデーション**などが、この「{item_name}」の製法で本当に実現可能か、あなたの専門知識を基に入念にチェックしてください。
...
def design(request):
    ...
    if '輪島塗' in item_name:
        tips = [
            "<ruby>漆<rt>うるし</rt></ruby>の表面を鋭く彫って金で輝く線を描く技法（<ruby>沈金<rt>ちんきん</rt></ruby>）や、金粉を蒔いて絵画のように盛り上がった模様を描く技法（<ruby>蒔絵<rt>まきえ</rt></ruby>）が使えます。",
            "実際に製造する際、職人が全て手作業で彫ったり描いたりするため、非常に細かい線や複雑な絵柄は難易度が格段に上がります。",
            "デザインは、器の立体的な形や曲面に沿わせ、模様のない「漆の余白」とのバランスを考えることが重要です。"
        ]
    elif '会津塗' in item_name:
        tips = [
            "銀粉や<ruby>色漆<rt>いろうるし</rt></ruby>などを重ねて描く<ruby>蒔絵<rt>まきえ</rt></ruby>や研ぎ出しが得意なので、絵柄や模様を活かしたデザインが向く。",
            "軽量で、フォルムの繊細さを生かせるため、線の細い模様やグラデーション表現がおすすめ。",
            "<ruby>加飾面<rt>かしょくめん</rt></ruby>が比較的薄い塗り層なので、厚い盛り上げや重ね塗りのデザインは避けると良い。"
        ]
    elif '有田焼' in item_name:
        tips = [
            "<ruby>白磁<rt>はくじ</rt></ruby>(白い磁器)の透明感が魅力なので、余白を活かしたデザイン（白を見せる構成）が美しく映える。",
            "細い線や繊細な構図が得意なため、精密な文様や整った<ruby>幾何学柄<rt>きかがくがら</rt></ruby>が向く。",
            "<ruby>釉薬<rt>ゆうやく</rt></ruby>(ガラスの膜)の下で発色する<ruby>呉須<rt>ごす</rt></ruby>（青い顔料）の線描が特徴なので、重ね塗りや厚い絵付けは避けると品のある仕上がりになる。"
        ]
        ...
        tips = [
            ...
            "あえて余白を残さず、器の全面を模様や色彩で大胆に埋め尽くす構図が、九谷焼らしい重厚さを生みます。"
        ]
    else:
        # デフォルトの注意点
        tips = [
